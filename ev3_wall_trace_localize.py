...
run_steps = 35 # no of motion steps in run

single_particle_mode = True # debugging mode with a single particle
known_starting_point = False # run particle filter with known starting point (always true for single particle mode)
...
sensor_noise_left = 2.0 # left sensor is normally within target_wall_dist of a wall
sensor_noise_front = 5.0 # front sensor is normally much further from a wall so accuracy goes down (NOT CURRENTLY USED)

if motion_noise_on:
    steering_noise = 0.001 # applies to any near straight motion or slight turn motion
    distance_noise = 0.05 # only applies to true straight motion (ex. after hard left turns at 45 deg)
    turning_noise = 0.05 # applies to turning in place (hard left or hard right turns)
else:
...
robust_likelihood_constant = 0.01 # avoids being overly aggressive on particle killing (sonar readings can be rubbish at times)
...
class robot_sim:
    ...
    def __init__(self):
        if single_particle_mode or known_starting_point:
            self.x = 35.6 # initial known x position for single robot simulator testing
            self.y = 7.6 # initial known y position for single robot simulator testing
            self.orientation = pi / 2 # initial known orientation for single robot simulator testing
        else:
    ...
    def move_time(self, pwr_l, pwr_r, duration):
        ...
        if velocity_left == velocity_right: # going straight
            ...
            orientation = (self.orientation + random.gauss(0.0, self.steering_noise)) % (2 * pi) # add steering noise
            dist = velocity_right * duration / 1000.0
            dist += random.gauss(0.0, self.distance_noise * dist) # add distance noise proportional to distance
            ...
            beta += random.gauss(0.0, self.steering_noise) # add steering noise
            # print 'beta is: ', beta
    ...
    def measurement_prob(self, measurements, grid):
        ...
        error = error_left + robust_likelihood_constant
